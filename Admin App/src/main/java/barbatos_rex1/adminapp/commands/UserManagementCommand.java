...
public class UserManagementCommand {
    ...
    public AttributedString listUsers(@ShellOption(help = "Flags for list command", defaultValue = "iesfr") String flags) {
        var allUsers = userService.all();
        StringBuilder sb = new StringBuilder();
        for (UserDTO user : allUsers) {
            buildWithFlags(sb, user, flags);
        }
        System.out.println("\nRegistered Users:\n");
    ...
    private void buildWithFlags(StringBuilder sb, UserDTO user, String flags) {
        var conditions = manageFlags(flags);
        if (conditions.id) {
        ...
        for (char c : flags.toCharArray()) {
            switch (c) {
                ...
                case 'r':
                    ...
                    break;
                default:
                    System.out.println(new AttributedString(String.format("[WARNING] Flag '%s' is not valid, and will be ignored.", c), AttributedStyle.BOLD.foreground(AttributedStyle.YELLOW)));
            }
        }
        return result;
    }
