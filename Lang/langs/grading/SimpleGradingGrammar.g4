...
short:  DEFINE GRADING FOR SHORT_AWNSER QUESTION id=STRING MARK (MAY ACCEPT COL attach_text_options AS CORRECT MARK)? (OVERRIDE ANSWERS WITH REGEX regex=STRING MARK)? IF CORRECT THEN AWARD maxPoints=INT MARK ( IF WRONG AWARD minPoints=INT MARK)?;
